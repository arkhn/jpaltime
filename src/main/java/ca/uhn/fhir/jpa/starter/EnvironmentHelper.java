...
public class EnvironmentHelper {
	...
	public static Properties getHibernateProperties(ConfigurableEnvironment environment,
		...
		properties.putIfAbsent(AvailableSettings.IMPLICIT_NAMING_STRATEGY,
				SpringImplicitNamingStrategy.class.getName());
		properties.putIfAbsent(AvailableSettings.PHYSICAL_NAMING_STRATEGY,
				SpringPhysicalNamingStrategy.class.getName());
		// TODO The bean factory should be added as parameter but that requires that it
		// can be injected from the entityManagerFactory bean from xBaseConfig
		...
		Map<String, Object> hapiJpaPropertyMap = new HapiFhirLocalContainerEntityManagerFactoryBean(
				myConfigurableListableBeanFactory).getJpaPropertyMap();
		...
		if (Boolean.parseBoolean(String.valueOf(properties.get(HibernateOrmMapperSettings.ENABLED)))) {
			if (isElasticsearchEnabled(environment)) {
				properties.putIfAbsent(BackendSettings.backendKey(BackendSettings.TYPE),
			...
			if (properties.get(BackendSettings.backendKey(BackendSettings.TYPE))
					.equals(LuceneBackendSettings.TYPE_NAME)) {
				properties.putIfAbsent(BackendSettings.backendKey(LuceneIndexSettings.DIRECTORY_TYPE),
						LocalFileSystemDirectoryProvider.NAME);
				properties.putIfAbsent(BackendSettings.backendKey(LuceneIndexSettings.DIRECTORY_ROOT),
						"target/lucenefiles");
				...
				properties.putIfAbsent(BackendSettings.backendKey(LuceneBackendSettings.LUCENE_VERSION),
						Version.LATEST);

			} else if (properties.get(BackendSettings.backendKey(BackendSettings.TYPE))
				...
				Boolean refreshAfterWrite = environment.getProperty("elasticsearch.debug.refresh_after_write",
						Boolean.class);
